...
static void engine_draw_frame(struct engine* engine) {
	...
	for (y = 0; y < 8; y++) {
		for (x = 0; x < 2; x++) {
			...
			glColor4f(0.0, 0.05, 0.05, 1);
			SetRectF(pos, -1 + x * 1 + INNER_X_MARGIN + INNER_X_MARGIN/2.0, 1 - y*0.25 - INNER_Y_MARGIN - INNER_Y_MARGIN, 
				-1 + (x + 1) * 1 - INNER_X_MARGIN - INNER_X_MARGIN/2.0, 1 - (y + 1) * 0.25 + INNER_Y_MARGIN + INNER_X_MARGIN/2.0);
			glVertexPointer(2, GL_FLOAT, 0, pos);
			glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
		}
	}
...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event) {
	...
	if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) { //터치 인풋 
		if (AKeyEvent_getAction(event) == AKEY_EVENT_ACTION_DOWN) { //터치 행위에 대한 조건 (if 다운, else if 업, else move)
			...
			char temp[16];
			int length = sprintf(temp, "%d", index); // 3-> "3"
			SendFrameData(gh_socket, 1, temp, length + 1); //strlen(temp) +1
			SendFrameData(gh_socket, 2, (char *)&index, sizeof(int)); //strlen(temp) +1
...
void android_main(struct android_app* state) {
	...
	srv_addr.sin_addr.s_addr = inet_addr("192.168.0.3");
					...
					while (ASensorEventQueue_getEvents(engine.p_sensor_queue, &event, 1) > 0)
					{
						if (event.type == ASENSOR_TYPE_ACCELEROMETER) {
							...
							if (gyro_sensor_manager.SetGyroscopeData(event.data, event.timestamp, temp_data) == 1) {
								if(count == 0) SendFrameData(gh_socket, 22, (const char *)temp_data, sizeof(float) * 3);
								count = (count + 1) % 3;
							}
						}
					}
